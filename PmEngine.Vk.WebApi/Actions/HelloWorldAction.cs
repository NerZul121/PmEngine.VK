using PmEngine.Core;
using PmEngine.Core.BaseMarkups;
using PmEngine.Core.Interfaces;

namespace PmEngine.Vk.WebApi.Actions
{
    public class HelloWorldAction : IAction
    {
        public async Task<INextActionsMarkup?> DoAction(ActionWrapper currentAction, IUserSession user)
        {
            var result = new LinedMarkup();
            result.CreateLine();
            result.Add("Привет!", GetType(), new Arguments(new() { { "text", "Привет!" } }));
            result.Add("Hello!", GetType(), new Arguments(new() { { "text", "Hello!" } }));
            result.CreateLine();
            result.Add("Bonjour!", GetType(), new Arguments(new() { { "text", "Bonjour!" } }));

            user.Media = new string[] { "" };

            user.AddToOutput(currentAction.Arguments.Get<string?>("text") ?? "Приветик!");

            return result;
        }
    }
}